...
|---|------|-------------|--------|------------|
| R1 | ~~**No existing Rust ReqIF crate**~~ — **Resolved.** `req1-reqif` crate implemented with typed Rust structs, serde XML serialization/deserialization, and .reqifz archive support. Entity mapping layer in `req1-core::reqif` provides bidirectional conversion. API endpoints and CLI commands implemented. | ~~High~~ | ~~High~~ | Standalone `req1-reqif` crate implemented. Entity mapping (import/export) in `req1-core::reqif`. API routes (`POST .../reqif/import`, `GET .../reqif/export`) and CLI commands (`req1 import`, `req1 export`) implemented. |
...
| TD15 | **No import/export** | No CSV, XLSX, YAML import/export. | Phase 1: CSV/YAML export/import. | Partial — ReqIF complete (XML library + entity mapping + API routes + CLI commands); CSV/XLSX/YAML pending |
