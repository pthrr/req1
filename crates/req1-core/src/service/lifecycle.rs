...
impl LifecycleService {
    pub async fn create(
        ...
        let states_json =
            serde_json::to_value(input.states.unwrap_or_default())
                .map_err(|e| CoreError::Internal(format!("json error: {e}")))?;
        let transitions_json =
            serde_json::to_value(input.transitions.unwrap_or_default())
                .map_err(|e| CoreError::Internal(format!("json error: {e}")))?;
    ...
    pub async fn get(
        ...
        lifecycle_model::Entity::find_by_id(id)
            .one(db)
            .await?
            .ok_or_else(|| CoreError::NotFound(format!("lifecycle model {id} not found")))
    }
